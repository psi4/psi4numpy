...
print("Specific rotation @ %d nm (Length Gauge): %10.5lf deg/[dm (g/cm^3)]"% (omega_nm, specific_rotation_lg))
...
print("\t\t%s\t             %s\t                  %s\n" % (cart[0], cart[1], cart[2]))
for a in range(0,3):
    print(" %s %20.10lf %20.10lf %20.10lf\n" % ( cart[a], optrot_vg_om[3*a+0], optrot_vg_om[3*a+1], optrot_vg_om[3*a+2]))
...
print('\n CCSD Optical Rotation Tensor (Modified Velocity Gauge) @ %d nm'%omega_nm)
print("\t\t%s\t             %s\t                  %s\n" % (cart[0], cart[1], cart[2]))
for a in range(0,3):
