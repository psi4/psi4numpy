...
S = np.concatenate((np.concatenate((sapt.s('aa'),sapt.s('ab')),axis=1),np.concatenate((sapt.s('ba'),sapt.s('bb')),axis=1)),axis=0)
...
e1_full += 2*(np.einsum('aA,Aa->',sapt.potential('aa','B'),D_aa)+np.einsum('ab,ba->',sapt.potential('ab','B'),D_ba)) # B potential
e1_full += 2*(np.einsum('bB,Bb->',sapt.potential('bb','A'),D_bb)+np.einsum('ba,ab->',sapt.potential('ba','A'),D_ab)) # A potential
...
G_abrs = (np.einsum('ik,jl->ijkl',T_ar,O_bs) - np.einsum('il,jk->ijkl',T_as,O_br) + np.einsum('jl,ik->ijkl',T_bs,O_ar) 
         -np.einsum('jk,il->ijkl',T_br,O_as) + double_mod_eri_abrs)
