'''
A reference implementation of ADC(2) for the calculation of electron
affinities for a restricted Hartree-Fock reference. A spin orbital
...
e_ijab = e_mo[o,None,None,None] + e_iab[None]
...
# Construct the singles-singles (1p-1p) space
h_ss  = np.diag(e_mo[v])
h_ss -= einsum('ijac,ijbc->ab', t2, eri[o,o,v,v]) * 0.25
h_ss -= einsum('ijbc,ijac->ab', t2, eri[o,o,v,v]) * 0.25
...
diag = np.concatenate([np.diag(h_ss), -e_iab.ravel()])
arg = np.argsort(np.absolute(diag))
guess = np.eye(diag.size)[:,arg[:n_states]]

# Compute the EAs
e_ea, v_ea = davidson(matvec, guess, diag, tol=tol)
